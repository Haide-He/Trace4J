...
extern __thread int eventFd[MAX_EVENTS];
extern __thread unsigned long eventRDCnt[MAX_EVENTS];
extern __thread unsigned long eventTempCnt;
extern __thread int curEventId;
//extern __thread struct timespec sampletimeEnd;
//extern __thread struct timespec sampletimeStart;
...
__thread unsigned my_eax, my_edx, testa, testb, c = (1<<30);

void linux_perf_events_reset(){
    WP_ThreadData_t * tData = TD_GET();
    for (int i = 0; i < wpConfig.maxWP; i++) {
        if(tData->watchPointArray[i].isActive) {
            ioctl(tData->watchPointArray[i].fileHandle, PERF_EVENT_IOC_RESET, 0);
        }
    }
}
...
__thread uint64_t readCounter[MAX_EVENTS];

static void OnWatchPoint(int signum, siginfo_t *info, void *uCtxt) {
    //__asm__ volatile("rdpmc" : "=a" (my_eax), "=d" (my_edx) : "c" (c));
    //clock_gettime(CLOCK_MONOTONIC, &sampletimeEnd);
    for (int i = 1; i < curEventId; i++){
        assert(read(eventFd[i] , &(readCounter[i]), sizeof(uint64_t)) > 0);
    }
    WP_ThreadData_t *tData = TD_GET();
    if (!tData) return;
    ...
    //double f = ((double)sampletimeEnd.tv_sec*1e9 + sampletimeEnd.tv_nsec) - ((double)sampletimeStart.tv_sec*1e9 + sampletimeStart.tv_nsec);
    //fprintf(stdout,"method run time: %f ns\n",f);
    ...
    //unsigned long debugEnd = (((unsigned long) my_eax) | (((unsigned long) (my_edx << 17)) << 15));
    //unsigned long debugTemp = ((((unsigned long) my_eax) | (((unsigned long) (my_edx << 17)) << 15)) - eventTempCnt);
    //unsigned long fixOverflow = 0;
//    if(debugTemp > 100000){
//        //fixOverflow = maxval - eventTempCnt;
//        fprintf(stderr, "debugtemp = %lu; eventTempCnt = %lu; debugEnd = %lu; tid = %d\n", debugTemp, eventTempCnt, debugEnd, gettid());
//    }
    //assert(debugEnd > eventTempCnt);
    //eventRDCnt[1] += ((((unsigned long) my_eax) | (((unsigned long) (my_edx << 17)) << 15) ) - eventTempCnt);
    for (int i = 1; i < curEventId; i++){
        eventRDCnt[i] += (readCounter[i] - eventTempCnt);
        assert(read(eventFd[i] , &eventTempCnt, sizeof(uint64_t)) >= 0);
    }
    //eventTempCnt = 0;
//    linux_perf_events_reset();
//    unsigned long debugtemp;
//    assert(read(eventFd[1] , &debugtemp, sizeof(uint64_t)) >= 0);
    //fprintf(stderr, "debugEventTempCnt = %lu\n", eventTempCnt);



    if (wpConfig.isLBREnabled == false) {
    	retVal = tData->fptr(&wpt); //OnRetWatchPoint
    }
    else {
...
//    long long* forTest = (long long)sampleData->va;
//    fprintf(stderr, "Watch Address: %llx, IP: %llx\n", (long long)sampleData->va, (long long)*forTest);
