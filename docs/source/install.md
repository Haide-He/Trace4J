...
* A modern Linux system (kernel version ≥ 5.15) with performance-monitoring unit (PMU) support.
* A JDK (Java 8 or above) installed and available in your `PATH`.
* C/C++ tooling for building native components (e.g., GCC 8.3.1+, Make, CMake).
* `perf_event_open` support enabled in your kernel, and system permissions to use hardware counters (e.g., `CAP_SYS_ADMIN` or `perf_event_paranoid` appropriately configured).
* For the GUI and trace-processing pipeline: Python 3.10+, and a modern web browser.
