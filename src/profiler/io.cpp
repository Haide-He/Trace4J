...
int OUTPUT::writef(const char *fmt, ...) {
    ...
    va_end(arg);
    if(data_size >= LOCAL_BUFFER_SIZE) {
        va_start(arg, fmt);
        char *large_local_buf = (char *)malloc((data_size) * sizeof(char));
        //std::cout <<  std::endl <<  data_size <<  "; string len: " << strlen(large_local_buf) << std::endl;
        //std::cout << "reach?" << std::endl;
        if (large_local_buf == NULL) assert(false);
        vsnprintf(large_local_buf, data_size, fmt, arg);
        //std::cout << "reach2 ?" << std::endl;
        int size_written = writeb(large_local_buf);
        free(large_local_buf);
